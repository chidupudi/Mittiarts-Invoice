// api/send-full-invoice-sms.js - Pertinax SMS API for Full Invoice Payment with DLT Template
export default async function handler(req, res) {
  ...
  try {
    console.log('📱 Pertinax Full Invoice SMS Request:', {
    ...
    if (!phoneNumber || !customerName || !totalAmount) {
      return res.status(400).json({
        success: false,
        error: 'Missing required fields: phoneNumber, customerName, totalAmount'
      });
    }
...
Please visit our web application for the latest collections: www.mittiarts.com`;
    ...
    console.log('🔧 API URL:', apiUrl.toString().substring(0, 100) + '...');
